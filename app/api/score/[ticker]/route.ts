...
// clamp qui accepte null et borne les nombres
...
function computeFundamentalsFromV10(r: any): Fundamentals {
  ...
  const cashFD = num(r?.financialData?.totalCash?.raw ?? r?.financialData?.totalCash);
  const debtFD = num(r?.financialData?.totalDebt?.raw ?? r?.financialData?.totalDebt);
  ...
  // éléments BS utiles pour fallback Invested Capital
  const currLiab0 = num(bsh?.[0]?.totalCurrentLiabilities?.raw ?? bsh?.[0]?.totalCurrentLiabilities);
  const cashBS0   = num(bsh?.[0]?.cash?.raw ?? bsh?.[0]?.cash);
  const shortInv0 = num(bsh?.[0]?.shortTermInvestments?.raw ?? bsh?.[0]?.shortTermInvestments);
  const cashLike0 = (cashBS0 ?? 0) + (shortInv0 ?? 0);
  ...
  if (cashFD != null && debtFD != null) netCash = cashFD - debtFD > 0 ? 1 : 0;
  ...
  // NOPAT ≈ OperatingIncome * (1 - taxRate), fallback ≈ NetIncome
  const taxRate =
  ...
  // InvestedCapital – Option 1: Debt + Equity - Cash (FD)
  const invested1 = (debtFD ?? 0) + (eq0 ?? 0) - (cashFD ?? 0);

  // InvestedCapital – Option 2 (fallback BS): TotalAssets - (Cash+ST Inv) - Current Liabilities
  let invested2: number | null = null;
  if (assets0 != null) {
    invested2 = assets0 - (cashLike0 || 0) - (currLiab0 ?? 0);
  }

  // Choix robuste : on prend le meilleur positif. Si aucun positif, null.
  const candidates = [invested1, invested2 ?? Number.NEGATIVE_INFINITY].filter((x) => Number.isFinite(x));
  const bestPositive = candidates.filter((x) => x > 0).sort((a, b) => b - a)[0];
  const invested = bestPositive ?? null;

  const roic = (nopat != null && invested != null && invested > 0)
    ? clampN(nopat / invested, -1, 1)
    : null;
