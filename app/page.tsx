...
export default function Page() {
  ...
  // === Nouvelle source de vérité pour les couleurs UI: le verdict ===
          ...
          <div className="lg:col-span-2">
            <div className="relative overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-b from-slate-900/60 to-slate-900/30">
              ...
              <div className="absolute -right-14 top-6 rotate-45">
                <div
                  className={`px-16 py-1 text-xs tracking-wider text-white/90 ${verdictColor(
                    data.verdict
                  )}`}
                >
              ...
              <div className="p-6 md:p-7">
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <RatioCard label="ROE" value={data.ratios?.roe} isPct />
                    <RatioCard label="ROA" value={data.ratios?.roa} isPct />
                    <RatioCard label="FCF / RN" value={data.ratios?.fcf_over_netincome} isPct />
                    <RatioCard label="ROIC (approx.)" value={data.ratios?.roic} isPct />
                  </div>
                </div>
              </div>
            </div>
...
function RatioCard({ label, value, isPct = false }: { label: string; value?: number | null; isPct?: boolean }) {
  const display =
    value == null
      ? "—"
      : isPct
  ...
  return (
    <div className="p-4 rounded-2xl bg-slate-900/40 border border-slate-800">
      ...
      <div className="text-xl font-semibold tabular-nums mt-1">{display}</div>
    </div>
  );
}
...
function SourceChip({ src }: { src: string }) {
  ...
  const variant =
    ...
    /yahoo:html|yahoo:v7|yahoo:summary|yahoo:v10/.test(src) ? "default" :
